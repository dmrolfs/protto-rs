...
use compatibility::StrategyCompatibilityTester;
...
impl FieldConversionMigration {
    ...
    fn generate_with_old_system_only(
        ...
        self.generate_field_conversions(field, ctx)
    ...
    fn validate_against_old_system(
        ...
        let old_result = self.generate_with_old_system_only(field, ctx)?;
...
fn get_global_migration() -> &'static FieldConversionMigration {
...
mod tests {
    ...
    fn test_environment_configuration() {
        unsafe {
            std::env::set_var("PROTTO_MIGRATION_MODE", "new_only");
        }
