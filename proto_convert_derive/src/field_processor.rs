...
use crate::proto_inspection::FieldOptionality;
...
pub fn generate_from_proto_field(
    ...
    let _trace = CallStackDebug::with_context(
        ...
        ctx.field_name,
        &[
            ("rust_field_type", &quote!(#field_type).to_string()),
    ...
    // Type analysis debug
    let is_option = type_analysis::is_option_type(ctx.field_type);
    let is_vec = type_analysis::is_vec_type(ctx.field_type);
    let is_primitive = type_analysis::is_primitive_type(ctx.field_type);
    let has_transparent = attribute_parser::has_transparent_attr(field);
    let has_proto_ignore = attribute_parser::has_proto_ignore(field);
    let field_type = ctx.field_type;

    _trace.type_analysis(
        &quote!(#field_type).to_string(),
        &[
            ("is_option", &is_option.to_string()),
            ("is_vec", &is_vec.to_string()),
            ("is_primitive", &is_primitive.to_string()),
            ("has_transparent", &has_transparent.to_string()),
            ("has_proto_ignore", &has_proto_ignore.to_string()),
        ],
    );

    let result = if has_proto_ignore {
        _trace.decision("has_proto_ignore", "generate_ignored_field");
        generate_ignored_field(ctx)
    } else if let Some(from_with_path) = attribute_parser::get_proto_derive_from_with(field) {
        _trace.decision(
            "has derive_from_with",
            &format!("use path: {}", from_with_path),
        );
        generate_derive_from_with_field(ctx, &from_with_path)
    } else if has_transparent {
        _trace.decision("has_transparent", "generate_transparent_field");
        generate_transparent_field(ctx)
    } else if is_option {
        _trace.decision("is_option_type", "generate_option_field");
        _trace.checkpoint_data(
            "is_option",
            &[(
                "inner_type",
                &quote!(type_analysis::get_inner_type_from_option(ctx.field_type)).to_string(),
            )],
        );
        generate_option_field(ctx)
    } else if is_vec {
        _trace.decision("is_vec_type", "generate_vec_field");
        _trace.checkpoint_data(
            "is_vec",
            &[(
                "inner_type",
        ...
        generate_vec_field(ctx)
    } else if let syn::Type::Path(_) = ctx.field_type {
        _trace.decision("is_path_type", "generate_path_type_field");
        generate_path_type_field(ctx, field)
    } else {
        _trace.error(&format!("Unsupported type for field '{}'", ctx.field_name));
        panic!(
...
pub fn generate_from_my_field(
    ...
    if attribute_parser::has_proto_ignore(field) {
        _trace.checkpoint("field ignored - returning empty");
...
fn generate_path_type_field(
    ...
    if let syn::Type::Path(type_path) = field_type {
        ...
        if is_enum {
            _trace.decision("is_enum", "generate_enum_field");
        ...
        match ctx.expect_mode {
            ...
            ExpectMode::None => {
                ...
                maybe_option_expr(
                    ...
                    quote! {
                        #proto_field_expr
                            .expect(&format!("Proto field {} is required", stringify!(#proto_field_expr)))
                            #conversion_suffix
                    },
                )
            }
...
fn generate_path_type_from_my_field(
    ...
    if let syn::Type::Path(type_path) = ctx.field_type {
        ...
        return if type_analysis::is_enum_type_with_explicit_attr(ctx.field_type, field) {
            _trace.decision("is_enum", "generate_enum_from_my_field");
